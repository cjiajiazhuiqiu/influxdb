...
func getLogCreatedUpdated(ctx context.Context, svc influxdb.KeyValueLog, id influxdb.ID, r influxdb.ResourceType) (createdAt, lastUpdatedAt time.Time, err error) {
	key := logKey(r, id)
	_, createdAt, err = svc.FirstLogEntry(ctx, key)
	if err != nil {
		return createdAt, lastUpdatedAt, err
	}
	_, lastUpdatedAt, err = svc.LastLogEntry(ctx, key)
	if err != nil {
		return createdAt, lastUpdatedAt, err
	}
	return createdAt, lastUpdatedAt, err
